...
def main(args):
    ...
    def pre_rank_run(cancer_path, pert_type, pert_matrix, sig_info, permutate_time):
        ...
        prerank_nes = df(index=['NREC_profile', 'REC_profile'], columns=pert_matrix.columns)
        prerank_fdr = df(index=['NREC_profile', 'REC_profile'], columns=pert_matrix.columns)
        prerank_es = df(index=['NREC_profile', 'REC_profile'], columns=pert_matrix.columns)
        ...
        prerank_es.to_csv(gsea_home + cancer_path + '/enr_result_' + pert_type + '/es_sorted_by_NREC.csv', sep=',')
        prerank_nes.to_csv(gsea_home + cancer_path + '/enr_result_' + pert_type + '/nes_sorted_by_NREC.csv', sep=',')
        prerank_fdr.to_csv(gsea_home + cancer_path + '/enr_result_' + pert_type + '/fdr_sorted_by_NREC.csv', sep=',')
    ...
    # calculate shift ability
    shift_ability = prerank_es.copy()
    shift_ability['shift_ability'] = shift_ability['REC_profile'] - shift_ability['NREC_profile']
    shift_ability = shift_ability.sort_values(by='shift_ability', ascending=False)

    shift_ability.to_csv(poseidon_home + 'shift_ability/' + args.cancer_path + '_' + args.pert_type + '.csv', sep=',')

if __name__ == '__main__':
    ### Parse command line arguments
    parser = argparse.ArgumentParser(description='Calculating enrNREC, enrREC, shift_ability for input perturbation matrix')
